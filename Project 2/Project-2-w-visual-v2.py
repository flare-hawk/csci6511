...
class GraphColoringCSP:
    ...
    def select_unassigned_variable(self, assignment):
        ...
        if len(min_value_vertices)==1:        
            ...
            #Tie Breaking 
            #Maxmum Constraint Number and Constraint Vertices Holder
            max_cons=0
            max_cons_vertices=[]
...
def draw_colors(num_colors):
    colors={}
    #random_seed
    random.seed(12345)
    for i in range(1,num_colors+1):
        while True:
            tmp_color="#"+("%06x" % random.randint(0, 0xFFFFFF))
            if tmp_color not in colors.values():
                colors[i]=tmp_color 
                break
    return colors
...
if __name__=="__main__":
    ...
    if colors is None:
        ...
        #Print Graphs for Visual Inspection:
        G=nx.Graph()
        G.add_edges_from(edges)
        sorted_vertices=sorted(vertices)
        colorramp=draw_colors(num_colors)
        sorted_colors=[]
        for i, v in enumerate(sorted_vertices):
            sorted_colors.append(colorramp[colors[v]])
        nx.draw_networkx(G, nodelist=sorted_vertices, node_color=sorted_colors)
        plt.show()
